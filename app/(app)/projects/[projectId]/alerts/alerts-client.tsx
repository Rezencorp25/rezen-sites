...
export default function AlertsClient({ projectId }: { projectId: string }) {
  ...
  const taskByAlertId = useMemo(() => {
    const map: Record<string, string> = {};
    for (const t of allTasks) {
      if (t.fromAlertId) map[t.fromAlertId] = t.id;
    }
    return map;
  }, [allTasks]);
  ...
  return (
    <div className="mx-auto max-w-5xl px-10 py-10">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {taskByAlertId[a.id] && (
                    <a
                      ...
                      className="ml-2 mt-2 inline-flex items-center gap-1.5 rounded-md bg-molten-primary/15 px-2 py-0.5 text-label-sm text-molten-primary hover:bg-molten-primary/25"
                    >
                      → task #{taskByAlertId[a.id].slice(-6)}
                    </a>
                  )}
                </div>
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        a.fixAction === "auto"
                          ...
                          : "bg-surface-container-lowest text-molten-primary hover:bg-surface-container-low",
                      )}
                      title={
                        a.fixAction === "auto"
                          ? "Risolvibile automaticamente"
                          : "Crea task per intervento manuale"
                      }
                    >
      ...
      <AlertFixDialog
        ...
        alert={fixOpen}
        onActionComplete={(_, _taskId) => {
          if (fixOpen) ackAlert(fixOpen.id);
        }}
      />
    </div>
  );
