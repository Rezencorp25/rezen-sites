...
/**
 * Import pagina da URL Framer pubblico.
 * Fetch HTML → estrae meta + primi heading/paragraph → brief per Page Designer.
 */
export async function POST(req: Request) {
  try {
    ...
    const brief = {
      goal: snapshot.h1 || snapshot.title || "Pagina importata da Framer",
    ...
    const { page, modes } = await orchestratePage({
      projectId,
      title: title || snapshot.title || "Framer import",
      slug: slug || toSlug(snapshot.title ?? "framer-import"),
      brief,
    });
    ...
    return NextResponse.json(
      { error: (err as Error).message ?? "framer-url import failed" },
      { status: 500 },
    );
  }
}
